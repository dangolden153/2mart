    ...
    db.ref("/tradeRequests/" + this.state.user.uid)
      ...
      .on("value", (snap) => {
        ...
        snap.forEach((request) => {
          ...
          let datePersonalized =
            startDate.getDate() +
            " " +
            this.getMonth(startDate.getMonth()) +
            " " +
          ...
          if (request.val().messages) {
            ...
            lastMessage =
              request.val().messages[messageKeys[messageKeys.length - 1]]
                .message;
          }

          chatList.push({
            name: request.val().cardName
              ? request.val().cardName
              : "Withdrawal",
    ...
    return (
      <ScrollView>
        {this.state.loading ? (
          <View style={{ padding: 20 }}>
            ...
            <Text style={{ textAlign: "center", padding: 15 }}>
              Please wait while we fetch chats for you
            </Text>
          </View>
        ) : this.state.chatList.length < 1 ? (
          <Text style={{ textAlign: "center", padding: 15 }}>
            You have no existing chats
          </Text>
        ) : null}
        {this.state.chatList.length > 0 && this.state.hasDeleted === true ? (
          ...
          >
            <Text style={{ paddingRight: RFValue(10), fontSize: RFValue(12) }}>
              Show Deleted?
            </Text>
            <Switch
              ...
              onValueChange={(val) => {
                this.setState({ showDeleted: val }, () => {
                  console.warn(this.state.showDeleted);
                });
              }}
            />
          </View>
