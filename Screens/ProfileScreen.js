...
// import { Icon, Input, Item } from "react-native-elements";
...
class ProfileScreen extends Component {
  ...
  getUserDetails = () => {
    db.ref("users/" + this.state.user.uid).on("value", (snap) => {
      console.log("user details", snap.val());
      this.setState({
              ...
              imageRef.getDownloadURL().then((url) => {
                ...
                db.ref("/users/" + this.state.user.uid)
                  ...
                  .catch((err) => {
                    Alert.alert(
                      "Error",
                      "There was an error uploading dp, please try again."
                    );
                  });
              });
            });
  ...
  changeNumber = () => {
    if (
      this.state.numberUpdated.length < 3 ||
      this.state.numberUpdated === this.state.number
    ) {
      ...
      db.ref("/users/" + this.state.user.uid)
        ...
        .catch(() => {
          Alert.alert(
            "Error",
            "There was an error updating number, please try again later."
          );
          this.setState({
            edit: false,
          });
      ...
      .then((res) => {
        ...
        db.ref("/requestLists/" + res.key)
          ...
          .then((res) => {
            Alert.alert(
              "Success",
              "Withdraw Request has been generated successfully. Kindly check chats to continue with the request."
            );
            this.props.navigation.navigate("ChatScreen"); //// chat ini
          })
          .catch((err) => {
            Alert.alert(
              "Error",
              "There was an error generating withdraw request"
            );
          });
      })
      .catch((err) => {
  ...
  logout = () => {
    auth
      .signOut()
      .then((res) => console.log("successfully logged out", res))
      .catch((e) => console.log("logout unsuccessfully!", e));
    //   .then(() => {
  ...
  //     message:
  //       "Download Tech2Smart app, use my referral code '" +
  //       this.state.referralId +
  //       "' to register and get 10 free reward tokens",
      ...
      <View style={{ width: "100%", flex: 1, paddingTop: RFValue(20) }}>
        ...
        >
          ...
          <View style={{ paddingLeft: RFValue(16), flex: 1 }}>
            ...
            >
              Total Transaction: {this.state.successful + this.state.failed}
            </Text>
          </View>
        </View>
        <View
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            ...
            />
            {this.state.edit ? (
              <View style={{ marginLeft: RFValue(17) }}>
                <Input
                  style={{
                    flex: 1,
                    color: "rgb(100,100,100)",
                ...
                >
                  <MaterialIcons
                    name="check"
                    // type="MaterialIcons"
                    style={{ fontSize: RFValue(18), color: "rgb(100,100,100)" }}
                  />
                </TouchableOpacity>
              </View>
            ) : (
          ...
          >
            <EvilIcons
              // type="EvilIcons"
              name="envelope"
              style={{ color: "rgb(100,100,100)", fontSize: RFValue(25) }}
            />
            <Text
              style={{
        ...
        >
          <View>
            ...
            >
              ...
              >
                ...
                <Text style={{ fontWeight: "500", paddingLeft: RFValue(20) }}>
                  Request Withdrawal
                </Text>
              </View>
            </TouchableHighlight>
          ...
          >
            ...
            >
              <Feather
                name="power"
                style={{ fontSize: RFValue(25), color: "#f44336" }}
              />
              <Text
                style={{
                  fontWeight: "500",
