...
import { Select, Icon, Input } from "native-base";
// import { Icon, Picker, Label, Input } from "react-native-elements";
...
export default class CardDetailsScreen extends Component {
  ...
  componentDidMount() {
    ...
    // work on the AsyncStorage from the other Library
    ...
    //           this.setState({
    //             selectedCard: cardObj[i],
    //             types: cardObj[i].types ? cardObj[i].types : [],
    //           });
  ...
  getUserDP = () => {
    db.ref("/users/" + this.state.user.uid + "/dp")
      .once("value")
      .then((res) => {
        this.setState(
          {
  ...
  onValueChange3 = (value) => {
    ...
    for (let i = 0; i < types.length; i++) {
      if (
        types[i].currency === value &&
        types[i].type.toLowerCase() === this.state.type.toLowerCase()
      ) {
        if (rangeList.indexOf(types[i].range) < 0) {
          rangeList.push(types[i].range);
        }
  ...
  onValueChange4 = (value) => {
    ...
    for (let i = 0; i < types.length; i++) {
      if (this.state.cardType === "crypto") {
        ...
        types[i].range === value &&
        types[i].currency === this.state.currency &&
        types[i].type.toLowerCase() === this.state.type.toLowerCase()
      ) {
        dailyRate = types[i].rate;
      }
  ...
  getCardType = (check) => {
    if (this.state.types.length > 0) {
      ...
      return (
        <Text style={{ fontSize: RFValue(11), fontWeight: "300" }}>
          Card Unavailable at the moment
        </Text>
      );
    }
  };
    ...
    if (Platform.OS === "ios")
      return (
        <View>
          ...
          {/* item check the closing view tag */}
          <View picker style={{ marginTop: RFValue(8) }}>
            {/* <Picker
                            mode="dropdown"
                            iosIcon={<Icon name="arrow-down" />}
                            style={{ width: undefined }}
      ...
      return (
        ...
        >
          ...
          >
            ...
            {this.state.rangePicker.map((value) => (
  ...
  startTrade = () => {
    ...
    let calculatedValue =
      "N" +
      parseFloat(
        Math.round(this.state.cardValue * this.state.dailyRate * 100) / 100
      ).toFixed(2);
  ...
  executeTrade = () => {
    db.ref("/tradeRequests/" + this.state.user.uid)
      .push({
        ...
        currency:
          this.state.cardType === "crypto" ? "USD" : this.state.currency,
      ...
      .then((res) => {
        ...
        db.ref("/requestLists/" + res.key)
          ...
          .then((res) => {
            Alert.alert(
              "Success",
              "Trade Request has been generated successfully. Kindly check chats to continue with the request."
            );
            ...
            this.props.navigation.navigate("Chat", {
              ...
              cost: parseFloat(
                Math.round(this.state.cardValue * this.state.dailyRate * 100) /
                  100
              ).toFixed(2),
          ...
          .catch((err) => {
            ...
            Alert.alert(
              "Error",
              "Failed to generate trade request, please try again later."
            );
          });
      })
      .catch((err) => {
  ...
  render() {
    return (
      <View style={{ flex: 1, width: "100%" }}>
        ...
        >
          <Text style={{ fontWeight: "bold", fontSize: RFValue(13) }}>
            Trade {this.state.cardType === "crypto" ? "bitcoin" : "gift cards"}
          </Text>
          <TouchableOpacity
            onPress={() => {
              this.props.navigation.goBack();
        ...
        <View style={{ flexDirection: "row", flex: 1, alignItems: "center" }}>
          <View style={{ width: RFValue(70) }}>
            <Text style={styles.detailsText}>
              {this.state.cardType === "crypto" ? "Bitcoin" : "Card"} Details
            </Text>
          </View>
          <KeyboardAwareScrollView
            ...
            style={{ flex: 1 }}
          >
            {this.state.cardType === "crypto"
              ? (this.state.cardTypeSelected = true)
              : this.getCardType()}

            {this.state.cardType === "crypto"
              ? (this.state.cardCountrySelected = true)
              : this.state.cardTypeSelected
              ? this.getCardCountry()
              : null}
        ...
        >
          ...
          <TouchableOpacity
            ...
            style={[
              ...
              {
                backgroundColor: this.state.color,
                opacity: this.state.cardRangeSelected ? 1 : 0.3,
              },
            ]}
